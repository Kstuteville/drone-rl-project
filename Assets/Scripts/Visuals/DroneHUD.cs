// DroneHUD.cs — In-flight telemetry display
// Original by Jessenth, integrated into shared architecture.
// Reads directly from Transform + Rigidbody (no architecture coupling).
...
public class DroneHUD : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        speed = droneRb.velocity.magnitude;
    ...
    void DrawBar(float x, float y, float w, string label, float value, Color color, bool centered)
    {
        ...
        else
        {
            ...
            float barW = Mathf.Min(clamped * w, w);
            GUI.color = color;
